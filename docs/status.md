...
> **Issue numbers changed.** All 47 issues moved to `Corelantic-Intelligence/ZainDevWork` and were renumbered — GitHub cannot transfer issues between a user-owned and an org-owned repo, so they were recreated. Every `#N` below is the **new** number. Commit messages and PR descriptions written before the move still carry the old ones; the mapping is pinned as an issue in the new repo.

## The headline

**There is a review deployment, and the dashboard is a full executive surface.** The stack runs as three containers on an EC2 host at **`98.91.116.58`** behind one nginx entry point with basic auth (#43) — on **sample data**, because the host cannot reach the warehouse yet (#46). As of PR `aaron24estrada/Corelantic#62` the app is a top-bar executive report — a revenue hero, a KPI stat band, the intake funnel, channel/geo breakdowns, and a **net-new Call Center section** over the telephony registry (Zoom calls + agent performance) that the embedded dashboard never showed. Run locally against `azure_sql` it renders end-to-end on live `gold_tspot` with real figures (~$224M revenue, 130k leads, 28.7% voucher rate, 80.7% answer rate).

**Two gates before live data reaches that host**: TLS (#44 — it serves plaintext HTTP today, so basic-auth credentials cross in the clear) and the access grant (#46).

**The backend serves 26 real metrics** over KRW's live Azure SQL (`gold_tspot`) through `POST /api/v1/query`, and the same registry runs against a seeded fixture that reproduces the source's numbers. Every metric on KRW's Executive Summary is reachable **except Spend and ROAS** — and that gap is an access grant (#31), not code.

One live-data gotcha now handled: SQL Server returns ratio/derived metrics as high-precision decimal **strings** (counts come as numbers), which a `typeof === "number"` guard silently dropped to "—" on Azure while the fixture's SQLite floats hid it. A shared `toNumber` in `lib/format` coerces at every read (value extractors, chart data, tooltips). The Azure dashboard is also **slow and erratic** (12–130s) — connection/token overhead per request plus `stages` distinct-counts; tracked in **#42**, fixture stays instant.
...
Deploying is `git pull && docker compose up -d --build`. Switching to live data is an `.env` edit (`CORELANTIC_API_DATA_SOURCE=azure_sql` plus the service-principal credentials) and `docker compose up -d` — no rebuild — but **not before #44 and #46**.

Two things about it that will bite if forgotten: SSH is scoped to a single IP that is currently the **KRW VPN egress**, so dropping the VPN loses SSH until the security group is updated; and port 80 is open to the world (#45).
...
- **#42 — live Azure SQL is slow & erratic (12–130s).** Per-request connection/token overhead plus `stages` distinct-counts; even a bare `COUNT` has taken 65s cold. Not blocking (fixture is instant) but it undercuts a live demo. Owners: us (pool warming, dashboard concurrency, caching slow-moving scalars) + Imran/KRW (warehouse indexing / gold-layer pre-agg).
...
0. **#45 — narrow port 80 to the reviewers' IPs.** Minutes of work, and the only thing standing in front of the host today is plaintext basic auth. Do this while the review is running.
...
2. **#42 — live Azure SQL perf.** The dashboard is verified correct on `gold_tspot` but slow/erratic; pool warming + fewer/batched dashboard queries + a small cache for all-time scalars would make a live demo snappy. Fixture is the fast path meanwhile.
3. **#16 remainder — the funnel *shape* and the US map.** The funnel already ships as bars; the tapered `FUNNEL` shape and the `MAP` are new `ChartType`s (extend `ChartSpec`, register in `components/chart.tsx`, do **not** add a bespoke component). `FUNNEL` also needs `stage_name` to declare its members in order; the map needs a US-states geoJSON asset. The trends combo still waits on spend (#31).
4. **A1 — CI running `make check` on PRs.** CI will need `msodbcsql18` + `unixodbc-dev` to build `pyodbc`. The suite is hermetic now (#34): it ignores `.env` and every `CORELANTIC_API_*` variable, so CI can set whatever it likes.
5. **#37 / #36 — partial-bucket family.** `accumulate` vs display window (#37) and `compare`'s `LAG` skipping empty periods (#36); fix before the D6 date-range control lands.
6. **#49 — component tests.** A cleanup pass dropped the zero guards in `bar-list` and `funnel`, rendering `NaN%` and `Infinity%`, and **`make check` stayed green**: a review caught it, not the suite. `apps/web` has no component tests at all.
7. **#47 — vendor Open Sans**, so image builds stop depending on Google Fonts. **#48 — build off-box (ECR + instance role)**, which removes the GitHub credential, the build cache and the build-memory constraint from the host in one move.

Gated: spend/referrals registry (#31), live data on the host (#46), TLS (#44), NL agent wiring (E1), auth (O-4).
...
- **Probe with a value that changes behaviour.** The hermetic-tests fix (#34) looked complete under `.env`, which only sets `data_source=fixture` — something the tests tolerate. Exporting `CORELANTIC_API_SEMANTIC_DIR=/tmp/nope` turned 24 tests red and exposed a fixture-ordering bug (pytest builds module-scoped fixtures before function-scoped autouse ones). A probe that cannot fail proves nothing.
