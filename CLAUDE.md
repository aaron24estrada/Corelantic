...
The MVP is specified in [`docs/`](docs/). **New here? Start with [`docs/status.md`](docs/status.md)** — where the build is, what's verified, what's blocked, and what to do next. Then [`docs/README.md`](docs/README.md) orders the rest. **Before touching the semantic layer / query engine / dashboard, read [`docs/concepts.md`](docs/concepts.md)** — the three ideas the design rests on (we model definitions not data; the model is visual-independent; the model is schema-bounded). The load-bearing decisions:
